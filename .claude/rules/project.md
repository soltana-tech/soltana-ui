...
The monorepo includes diagramming and framework integration
packages that extend the core design system:

| Package               | Purpose                                                                                           |
| --------------------- | ------------------------------------------------------------------------------------------------- |
| `@soltana-ui/tokens`  | Token compiler -- outputs Mermaid themes, DTCG JSON, agent docs YAML, llms.txt, llms-full.txt     |
...
The Mermaid integration is a convenience feature for documentation
and diagramming use cases. It reads live CSS custom properties at
runtime and maps them to Mermaid's theme config, keeping diagrams
in sync with the active color scheme. Relief and finish tiers do
not affect Mermaid's SVG rendering — those CSS-layer effects are
specific to DOM elements.

## Design System Reference

**Location**: `.claude/reference.yaml`
